        ...
        return async (req, res, next) => {
            ...
            if (!adminOnly) {
                try {
                    ...
                    if (fetchGuilds)
                        req.guilds = await this._fetchGuilds(client, token, req.user.id);
    ...
    static addRole({ client }) {
        return async (req, res, next) => {
            ...
            try {
                ...
                if (!guild) return res.status(500).json({ success: false, message: `Бот не найден на сервере` });
                ...
                const getIpInfo = await (await fetch(`http://ip-api.com/json/${ip}?fields=status,message,proxy,query`));
                ...
                if (!roleToGiveId) return res.status(400).json({ success: false, message: `Роль для этого сервера не найдена` });
    ...
    static removeRole({ client }) {
        return async (req, res, next) => {
            ...
            try {
                ...
                if (!roleToRemoveId) return res.status(400).json({ success: false, message: `Роль для этого сервера не найдена` });
