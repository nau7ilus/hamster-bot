  ...
  message.channel.send(
    guildData.give_role.message_type === 'plain_text'
      ? // eslint-disable-next-line max-len
        `**\`[🗑️ | Удаление]\` ${reactedMember} \`удалил запрос пользователя\` ${requestAuthor} \`с ником ${requestInfo.user.nick_info[0].replace(
          /[`|"|*]/gi,
          '',
        )}\`**`
      : new MessageEmbed()
          ...
          .setDescription(
            // eslint-disable-next-line max-len
            `**${reactedMember} удалил запрос пользователя ${requestAuthor} с ником \`${requestInfo.user.nick_info[0].replace(
              /[`|"|*]/gi,
              '',
            )}\`**`,
          ),
