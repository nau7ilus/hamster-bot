...
module.exports = new Command(
  ...
  async (client, message, args) => {
    ...
    const embed = new MessageEmbed().setColor("#beff33");
    let command;
    if (args[0]) {
      if ((command = await getThing(client, "command", args[0]))) {
        embed
          ...
          .setDescription(
            `Параметры: **<> - обязательные, [] - опционально**
...
${command.devOnly ? `**Доступно только для разработчиков**` : ""}`
        ...
        if (command.userPermissions) {
          embed.addField(
            "Необходимы права у пользователя:",
            `**${permsToRus(command.userPermissions).join(", ")}**`
          );
        }
        if (command.clientPermissions) {
          embed.addField(
            "Необходимы права у бота:",
            `**${permsToRus(command.clientPermissions).join(", ")}**`
          );
        }
        if (command.aliases) {
