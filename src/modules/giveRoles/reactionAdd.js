...
exports.run = async ({ client, reaction, reactedUser, guildSettings }) => {
  ...
  if (emoji.name == "🔎") {
    ...
    if (requestAuthor) {
      ...
      if (!embedNick) {
        ...
        if (
          embedNick !== requestAuthor.displayName ||
  ...
  if (emoji.name == "❌") {
    ...
    message.channel.send(
      guildSettings.give_role.message_type == "plain_text"
        ...
        : new MessageEmbed()
            ...
            .setDescription(
              `**${reactedMember} отклонил запрос пользователя ${ // prettier-ignore
    ...
    channel.send(guildSettings.give_role.message_type == "plain_text" ?
		// prettier-ignore
			`**\`[Отклонение] \`${requestAuthor}, ${ // prettier-ignore
				reactedMember}\` отклонил ваш запрос на получение роли \`${requestAuthor // prettier-ignore
			}\` с ником ${requestInfo.user.nick_info[0]}\`**` : // prettier-ignore
			new MessageEmbed()
				.setColor('#e34536')
				.setTitle("**❌ | Отклонение запроса**")
				.setDescription(`**${requestAuthor}, ${ // prettier-ignore
					reactedMember} отклонил ваш запрос на получение роли ${requestAuthor // prettier-ignore
					} с ником ${requestInfo.user.nick_info[0]}**`)) // prettier-ignore
